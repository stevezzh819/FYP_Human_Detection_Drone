...
def test_crazyflie_esp(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
...
def test_crazyflie_esp_linux(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_crazyflie_esp_macos(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
def test_crazyflie_esp_host(app: QemuApp, dut: QemuDut) -> None:
